...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-bold mb-0" style="color: #2c3e50; letter-spacing: -0.5px;">Tambah Artikel</h5>
            ...
            <form action="{{ route('artikel.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul"
                        name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul artikel" style="border-radius: 8px; font-size: 13.5px; padding: 10px 14px;">
                    @error('judul')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('isi') is-invalid @enderror" id="isi" name="isi" rows="6"
                        placeholder="Masukkan isi artikel" style="border-radius: 8px; font-size: 13.5px; padding: 10px 14px;">{{ old('isi') }}</textarea>
                    @error('isi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
